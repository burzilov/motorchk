...
Миграция полностью заменяет `themes/example/` версией из release, обновляет `engine_version` и очищает кеш. Пользовательские темы в `themes/{name}/` и контент (включая `content/_menus.yaml`) не изменяются.
