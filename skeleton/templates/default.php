...
>
    ...
    <header class="border-b border-slate-200 bg-white">
        <div class="mx-auto flex max-w-5xl items-center justify-between px-4 py-4">
            ...
            <button
                type="button"
                class="rounded border border-slate-300 px-3 py-1 text-sm md:hidden"
    ...
    <main class="mx-auto max-w-5xl px-4 py-10">
        <?php if (!empty($blocks['hero']) || !empty($canEdit)): ?>
            <section
                class="mb-10 rounded-2xl bg-white p-8 shadow-sm prose prose-slate max-w-none<?= !empty($canEdit) ? ' editable' : '' ?>"
                <?php if (!empty($canEdit)): ?>data-block="hero"<?php endif; ?>
            >
                <?= $blocks['hero'] ?? '' ?>
            </section>
        <?php endif; ?>
        ...
        <?php if (!empty($blocks['content']) || !empty($canEdit)): ?>
            <section
                class="rounded-2xl bg-white p-8 shadow-sm prose prose-slate max-w-none<?= !empty($canEdit) ? ' editable' : '' ?>"
                <?php if (!empty($canEdit)): ?>data-block="content"<?php endif; ?>
            >
                <?= $blocks['content'] ?? '' ?>
            </section>
        <?php endif; ?>
